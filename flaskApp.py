...
def detect_hand(frame):
    """ Detects the hand gesture and returns the predicted class (Rock/Paper/Scissors). """
...
def get_unique_computer_choice():
    """ Ensures the computer doesn't repeat the last choice. """
...
latest_user_choice = None  # Store the latest user choice


def generate_frames():
    """ Video streaming generator function that also stores the detected hand gesture. """
    ...
    while True:
        ...
        if hand_choice:
            latest_user_choice = hand_choice  # Store detected gesture
            winner_text = f"User: {hand_choice}"
        else:
            latest_user_choice = None  # No valid hand detected
...
def play():
    ...
    if not game_running:
        game_running = True  # Restart game if previously stopped

    user_choice = latest_user_choice  # Use the most recent detected gesture
...
def stop_game():
    """ Stops the game and declares the final winner. """
    # global game_running
    # game_running = False
